...
class Triangle:
    ...
    """
    def __init__(self, base_cur="eos", quote_cur="eth", mid_cur="usdt", interval=10):
        """
        初始化
        :param base_cur:  基准资产
...
#            print(huobi_market.market_detail(self.base_cur, self.quote_cur))
            ...
            '''
                ...
                对应的套利条件就是：
                ltc_usdt_buy_1_price >
                btc_usdt_sell_1_price*ltc_btc_sell_1_price*(1+btc_usdt_slippage)*(1+ltc_btc_slippage)
                /[(1-btc_usdt_fee)*(1-ltc_btc_fee)*(1-ltc_usdt_fee)*(1-ltc_usdt_slippage)]
                考虑到各市场费率都在千分之几的水平，做精度取舍后，该不等式可以进一步化简成：
                (ltc_usdt_buy_1_price/btc_usdt_sell_1_price-ltc_btc_sell_1_price)/ltc_btc_sell_1_price
                >btc_usdt_slippage+ltc_btc_slippage+ltc_usdt_slippage+btc_usdt_fee+ltc_usdt_fee+ltc_btc_fee
